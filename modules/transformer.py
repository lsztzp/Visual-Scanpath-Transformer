...
class Encoder(nn.Module):
    def __init__(self, feature_dim,
        ...
        self.layers = nn.ModuleList([EncoderLayer(d_model, d_k, d_v, n_heads, d_ff, dropout) for _ in range(enc_n_layers)])
...
class Decoder(nn.Module):
    def __init__(self, d_model, postion_method, max_length, d_k, d_v, n_heads, d_ff, dropout, dec_n_layers):
        ...
        self.tgt_emb = nn.Linear(2, d_model)
        if postion_method == 'fixed':
            self.pos_embed = FixedAbsolutePositionEmbedding(max_length, d_model, position_embedding_type='fixed')  # Fix 位置编码
        elif postion_method == 'learning':
            self.pos_embed = LearnableAbsolutePositionEmbedding(max_length, d_k * n_heads)  # learning 位置编码

        self.layers = nn.ModuleList([DecoderLayer(d_model, d_k, d_v, n_heads, d_ff, dropout) for _ in range(dec_n_layers)])
    ...
    def forward(self, enc_outputs, dec_inputs, enc_masks, dec_masks):
        ...
        if enc_masks is not None:
            ...
            dec_enc_attn_mask = get_attn_pad_mask(dec_masks,
                                                  torch.zeros(enc_outputs.size(0), enc_outputs.size(1))).to(enc_outputs.device)
...
class Transformer(nn.Module):
    def __init__(self, feature_dim,
        ...
        self.mdn = MDN(d_model, MDN_hidden_num, 2, num_gauss)
        self.eos = eos
        if eos:
    ...
    def forward(self, enc_inputs, enc_masks, dec_inputs, dec_masks):

        enc_outputs, enc_self_attns = self.encoder(enc_inputs, enc_masks)

        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(enc_outputs, dec_inputs, enc_masks, dec_masks)
        pi, mu, sigma, rho = self.mdn(dec_outputs)
        if self.eos:
            eos = self.eos(dec_outputs)
        else:
            eos = 0
